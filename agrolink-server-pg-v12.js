...
async function processVideoAsync(postId, inputPath, videoId) {
    ...
    if (activeVideoJobs >= MAX_CONCURRENT_VIDEOS) {
        // Kısa bekle ve tekrar dene (soft throttle)
    ...
    try {
        ...
        if (hasMedia) {
            ...
            if (isVideo) {
                ...
                // Gönderiyi hemen kaydet (processing placeholder)
                media     = `/uploads/videos/${videoId}.mp4`;
                mediaType = 'video';

                // Arka planda işle: optimize → HLS → thumbnail
                // postId henüz DB'ye yazılmadı, aşağıda enqueueVideoProcessing çağrılacak
                // videoId'yi dışarıya taşı
        ...
        // Arka plan video işleme kuyruğuna ekle
        if (req._pendingVideo) {
            const { videoId, tempPath } = req._pendingVideo;
            enqueueVideoProcessing(postId, tempPath, videoId);
            console.log(`🎬 Video kuyruğa eklendi: ${videoId}`);
        }
    ...
    try {
        const post = await dbGet(`
            SELECT p.*, u.username, u.name AS "userName", u."profilePic" AS "userProfilePic",
                   ...
                   (SELECT COUNT(*)::int FROM comments WHERE "postId"=p.id AND "isActive"=TRUE) AS "commentCount"
            FROM posts p JOIN users u ON p."userId"=u.id
